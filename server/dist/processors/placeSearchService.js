    ...
    return new Promise((resolve, reject) => {
        ...
        https.get('https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=' + lat + ',' + lng + '&radius=1500&type=restaurant&keyword=' + keyword + '&key=' + process.env.API_KEY, (resp) => {
            ...
            resp.on('end', () => {
                ...
                Place.find({ placeId: { "$in": cachedPlaces } }, (err, places) => {
                    if (err) {
                        reject(err);
                    }
                    resolve(places);
                ...
                try {
                    if (needAnalysisPlaces.length) {
                        invokeBatch(needAnalysisPlaces);
                        resolve([]);
                    }
                }
                catch (err) {
...
function invokeBatch(needAnalysisPlaces) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (anyErrorPlaceId.length) {
            anyErrorPlaceId.forEach(function (retryPlaceId) {
